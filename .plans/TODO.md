...
- [ ] **Re-evaluate `retentionPolicy: 10d` per app** — now backed by real data (2026-07-20/21 squeeze above): dropping to `1d` freed ~93% of the fleet's backup storage, meaning the overwhelming majority of the ~220 GiB was backups aged 1-10 days that are rarely, if ever, actually needed for recovery. Still deferred/undecided pending a real decision on acceptable recovery-window tradeoffs (barman-cloud plugin has no incremental/differential backup support today, so this is a straight days-retained tradeoff) — user has not agreed to permanently shorten it.

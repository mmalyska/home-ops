...
- [ ] **Migrate CNPG clusters from `system` to `standard` + barman-cloud plugin**
  ...
  - Other clusters (`litellm`, `gitea`, `keycloak`) use the deprecated plain `17.X` rolling tags — these should also migrate to `standard` + plugin when barman-cloud plugin is deployed

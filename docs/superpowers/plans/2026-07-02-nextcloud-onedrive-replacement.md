...
**Update from Task 8:** no new `nextcloud-s3-*` entries were created — all CNPG apps in this cluster (gitea, harbor, now nextcloud) share one S3 backup credential, so `s3-externalsecret.yaml` reuses gitea's existing UUIDs (`e00e1e38-ae37-479a-8b46-b409016331eb` / `4d5a418c-82ed-4b8e-bfbf-b40901634ea4`) instead of the two entries originally listed here.
